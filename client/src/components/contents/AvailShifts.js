...
class AvailShifts extends Component {
  ...
  _callApi = async () => {
    ...
    const orderedShiftsFromNow = orderedShifts.filter(
      shift => shift.startTime > now
    );
    const cities = await [
      ...new Set(orderedShiftsFromNow.map(item => item.area))
    ].sort();

    let cityObj = {};
    await cities.map(city => {
      cityObj[city] = orderedShiftsFromNow.filter(shift => shift.area === city);
    });
      ...
      for (let dateKey in cityObj[cityName]) {
        ...
        shiftRows.push(
          <AvailShiftsRow
            ...
            date={shortDate}
            shiftAmount={cityObj[cityName][dateKey].length}
