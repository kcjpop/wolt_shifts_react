...
const routes = [
  ...
  },
  {
    ...
    console.log('allShifts:', db.shifts.list())
      const allShifts = await db.shifts.list();

      const overlappingShiftExists = !!allShifts
        .filter(s => {console.log('s:',s);return s.booked})
        .find(s => s.startTime < shift.endTime && s.endTime > shift.startTime);

    console.log('overlappingShiftExists:', overlappingShiftExists)

      if (overlappingShiftExists) {
        throw Boom.badRequest('Cannot book an overlapping shift');
      }

      const settingBook = await db.shifts.set(params.id, { booked: true });
    console.log('settingBook:', settingBook)
      await delay(500);
    console.log('db.shifts.get(params.id):', db.shifts.get(params.id))
